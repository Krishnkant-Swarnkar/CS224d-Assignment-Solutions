...
class Config(object):
    ...
    """
    output_size = 2 # EDIT BY KRISHNKANT
    embed_size = 35
    label_size = 2
    early_stopping = 2
...
class RNN_Model():
    ...
    def load_data(self):
        ...
        all_sents = [t.get_words() for t in self.train_data]+[t.get_words() for t in self.dev_data]+[t.get_words() for t in self.test_data]
    ...
    def inference(self, tree, predict_only_root=False):
        ...
        if predict_only_root:
            ...
            node_tensors = [tf.reshape(tensor,(1,self.config.embed_size)) for node, tensor in node_tensors.iteritems() if node.label!=2]
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
            embedding = tf.get_variable('EM',(len(self.vocab), self.config.embed_size) ,tf.float32,init)
            W1 = tf.get_variable('W1',(2* self.config.embed_size, self.config.embed_size) ,tf.float32,init)
            b1 = tf.get_variable('b1',(1, self.config.embed_size) ,tf.float32,init)
            ### END YOUR CODE
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            U = tf.get_variable('U',(self.config.embed_size, self.config.output_size) ,tf.float32,init)
            bs = tf.get_variable('bs',(1, self.config.output_size) ,tf.float32,init)
            ### END YOUR CODE
        l2reg_loss = self.config.l2 * (tf.nn.l2_loss(W1)+tf.nn.l2_loss(U)+tf.nn.l2_loss(b1)+tf.nn.l2_loss(bs))
        tf.add_to_collection(name = 'l2_loss',value = l2reg_loss)
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ### YOUR CODE HERE
            embedding = tf.get_variable('EM',(len(self.vocab), self.config.embed_size) ,tf.float32)
            W1 = tf.get_variable('W1',(2* self.config.embed_size, self.config.embed_size) ,tf.float32)
            b1 = tf.get_variable('b1',(1, self.config.embed_size) ,tf.float32)
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            # print '*'*20,self.vocab.word_to_index[node.word],'#'*10
            curr_node_tensor = tf.nn.embedding_lookup(embedding,self.vocab.word_to_index[node.word])
            ...
            ### YOUR CODE HERE 
            W_left ,W_right = tf.split(W1,2,0)
            c_left  = tf.reshape(node_tensors[node.left],(1,self.config.embed_size)) 
            c_right = tf.reshape(node_tensors[node.right],(1,self.config.embed_size))
            curr_node_tensor = tf.nn.relu(tf.matmul(c_left,W_left)+tf.matmul(c_right,W_right)+b1)
            # p = [node_tensors[node.left],node_tensors[node.right]]
            # p = tf.convert_to_tensor(tf.concat(p,0))
            # p = tf.reshape(p,(1,2 * self.config.embed_size))
            # curr_node_tensor = tf.nn.relu(tf.matmul(p,W1)+b1)
            ### END YOUR CODE
    ...
    def add_projections(self, node_tensors):
        ...
        with tf.variable_scope('Projection') as scope:
            ...
            U = tf.get_variable('U',(self.config.embed_size, self.config.output_size) ,tf.float32)
            bs = tf.get_variable('bs',(1, self.config.output_size) ,tf.float32)
    ...
    def loss(self, logits, labels):
        ...
        loss += tf.get_collection('l2_loss')[0]
        # END YOUR CODE
        return loss
                ...
                for _ in xrange(RESET_AFTER):
                    ...
                    if verbose:
                        ...
                        sys.stdout.write('\r{} / {} :\tloss = {}'.format(step, 
                            len(self.train_data), np.mean(loss_history)))
                        sys.stdout.flush()
                    step+=1
                saver = tf.train.Saver()
