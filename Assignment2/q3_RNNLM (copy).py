...
class RNNLM_Model(LanguageModel):
  ...
  def __init__(self, config):
    ...
    self.output = tf.reshape(tf.concat( self.outputs,1 ), [-1, len(self.vocab)])
    # self.calculate_loss = self.add_loss_op(output)
    # self.train_step = self.add_training_op(self.calculate_loss)

  def setter(self,loss,train_op):
...
def test_RNNLM():
  ...
  loss = model.add_loss_op(model.output)
  train_operation = tf.train.AdadeltaOptimizer(config.lr).minimize(loss)
  model.setter(loss,train_operation)
