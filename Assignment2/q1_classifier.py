...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope('softmax-classifier') as scope:
      # scope.reuse_variables()
      W = tf.get_variable('weight',(self.config.n_classes,self.config.n_features),tf.float32,init)
      b = tf.get_variable('bias',(1,self.config.n_classes),tf.float32,init)
      out = softmax(tf.matmul(input_data,tf.transpose(W))+b)
    ### END YOUR CODE
    return out
